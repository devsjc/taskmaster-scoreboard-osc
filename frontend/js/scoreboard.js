...
// Create and open a websocket to communicate with the backend
const feSocket = new WebSocket("ws://localhost:8081");
feSocket.addEventListener('open', function (event) {
    feSocket.send('Hello Server!');
    wsConnected = true;
});

feSocket.addEventListener('closed', function(event) {
    wsConnected = false;
})

// Listen for messages
feSocket.addEventListener('message', function (event) {
    let msgDict = JSON.parse(event.data)
    console.log('Frontend: Message from server: ', msgDict);
    if (msgDict.address === "/play") {
        play()
    }
});
...
// Only show the play button if there isn't a websocket connection
function showPlay() {
    if (!wsConnected) {
